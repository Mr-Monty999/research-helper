<?php
return [

    "Latitude" => "خط العرض",
    "Longitude" => "خط الطول",
    "Location" => "الموقع",
    "Created At" => "أنشىء منذ",
    "User Name" => "إسم المستخدم",
    "Ambulance Requests" => "طلبات الإسعاف",
    "Blood Transfare Requests" => "طلبات نقل الدم",
    "Transfare Type" => "نوع النقل",
    "Blood Type" => "فصيلة الدم",
    "Bottels Count" => "عدد الزجاجات",
    "Hostpital Name" => "إسم المستشفى",
    "Transfare Time" => "زمن النقل",
    "Operation Type" => "نوع العملية",
    "Operation Type Detail" => "تفاصيل نوع العملية",
    "Distance" => "المسافة",
    "Follow Up Phone" => "رقم هاتف التتبع",
    "Accepted By" => "قُبل بواسطة",
    "Pharmacy Description" => "تفاصيل الصيدلية",
    "Is Alternative" => "هل هو بديل",
    "Pharmacy Phone" => "رقم هاتف الصيدلية",
    "Drug Request Id" => "رقم طلب الدواء",
    "Drug Requests Replies" => "ردود طلبات الدواء",
    "Name" => "الإسم",
    "Deleted At" => "مسح في",
    "Hospitals" => "المستشفيات",
    "Site Settings" => "إعدادت الموقع",
    "Login" => "تسجيل دخول",
    "Afeia" => "عافية",
    "Welcome Back" => "مرحبا بعودتك",
    "Enter Email Address" => "أدخل البريد الإلكتروني",
    "Password" => "كلمة المرور",
    "Remember Me" => "تذكرني",
    "Dashboard" => "لوحة التحكم",
    "Blood Transfare Requests List" => "قائمة طلبات نقل الدم",
    "Drug Requests" => "طلبات الدواء",
    "login-error" => "الرجاء التحقق من بيانات تسجيل الدخول",
    "Logout" => "تسجيل خروج",
    "Ready to Leave" => "مستعد للمغادرة؟",
    "Cancel" => "إلغاء",
    "id" => "الرقم",
    "receiver" => "مستلم",
    "urgent" => "عاجل",
    "birth" => "ولادة",
    "Hospitals List" => "قائمة المستشفيات",
    "Hospital Name" => "إسم المستشفى",
    "Actions" => "الأحداث",
    "Create Hospital" => "إضافة مستشفى",
    "Create" => "إضافة",
    "Hospital Added Successfully" => "تم إضافة المستشفى بنجاح",
    "Delete" => "حذف",
    "Edit" => "تعديل",
    "Update" => "تحديث",
    "Deleted Successfully" => "تم الحذف بنجاح",
    "Added Successfully" => "تم الإضافة بنجاح",
    "Updated Successfully" => "تم التحديث بنجاح",
    "Are you sure to perform this process" => "هل أنت متأكد من إجراء هذه العملية؟",
    "Delete Hospital" => "حذف مستشفى",
    "Ambulance Requests List" => "قائمة طلبات الإسعاف",
    "User Phone" => "رقم هاتف المستخدم",
    "Created Date" => "تاريخ الإنشاء",
    "Drug Requests List" => "قائمة طلبات الدواء",
    "Drug Name" => "إسم الدواء",
    "Drug Image" => "صورة الدواء",
    "No Image" => "لايوجد صورة",
    "Show All Replies" => "عرض جميع الردود",
    "Replies Count" => "عدد الردود",
    "Pharmacy Name" => "إسم الصيدلية",
    "Profile" => "الملف الشخصي",
    "Email" => "البريد الإلكتروني",
    "Phone" => "رقم الهاتف",
    "New Password" => "كلمة المرور الجديدة",
    "Old Password" => "كلمة المرور القديمة",
    "Old Password Is Not Correct" => "كلمة المرور القديمة غير صحيحة !",
    "Profile Updated Successfully" => "تم تحديث الملف الشخصي بنجاح",
    "Success" => "نجاح",
    "Hospital Location" => "موقع المستشفى",
    "Back" => "رجوع",
    "Profile Image" => "الصورة الشخصية",
    "Admins List" => "قائمة المشرفين",
    "Create Admin" => "إضافة مشرف",
    "Admins" => "المشرفين",
    "Image" => "الصورة",
    "Password Confirmation" => "تأكيد كلمة المرور",
    "Delete Admin" => "مسح مشرف",
    "Update Admin" => "تحديث المشرف",
    "volunteer" => "متبرع",
    "Reseach Help Requests" => "عدد طلبات المساعدة في البحث",
    "Levels List" => "قائمة المراحل",
    "Create Level" => "إضافة مرحلة",
    "Levels" => "المراحل",
    "Level Name" => "إسم المرحلة",
    "Description" => "الوصف",
    "Delete Level" => "حذف مرحلة",
    "Edit Level" => "تعديل مرحلة",
    "Sections" => "الأقسام",
    "Sections List" => "قائمة الأقسام",
    "Section Name" => "إسم القسم",
    "Delete Section" => "حذف القسم",
    "Edit Section" => "تعديل القسم",
    "Create Section" => "إضافة قسم",
    "Colleges" => "الكليات",
    "Colleges List" => "قائمة الكليات",
    "College Name" => "إسم الكلية",
    "Delete College" => "حذف كلية",
    "Edit College" => "تعديل كلية",
    "Create College" => "إضافة كلية",
    "Majors" => "التخصصات",
    "Majors List" => "قائمة التخصصات",
    "Major Name" => "إسم التخصص",
    "Delete Major" => "حذف التخصص",
    "Edit Major" => "تعديل التخصص",
    "Create Major" => "إضافة تخصص",
    "Help Types" => "أنواع المساعدة",
    "Help Types List" => "قائمة أنواع المساعدة",
    "Help Type Name" => "إسم نوع المساعدة",
    "Delete Help Type" => "حذف نوع المساعدة",
    "Edit Help Type" => "تعديل نوع المساعدة",
    "Create Help Type" => "إضافة نوع مساعدة",
    "Site Setttings" => "إعدادات الموقع",
    "Whatsapp Phone" => "رقم هاتف الواتساب",
    "Please Choose Your Level" => "الرجاء إختيار المرحلة !",
    "Please Choose Your College" => "الرجاء إختيار الكلية !",
    "Please Choose Your Major" => "الرجاء إختيار التخصص !",
    "Please Choose Your Section" => "الرجاء إختيار القسم !",
    "Please Choose Your Type Help" => "الرجاء إختيار نوع المساعدة"


































];
